...
const TicketList = ({ tickets = [] }) => {
  ...
  function Th({ children, sorted, reversed, onSort }) {
    ...
    return (
      <th>
        <div
          className="button is-borderless is-rounded has-icons-left is-small"
          onClick={onSort}
        >
      ...
      <tr className={checked ? "is-selected" : ""}>
          ...
          <div
            className={`tag ${
              ticket.ticket_status === "CHECKED IN"
                ...
                : "has-background-link"
            } has-text-info-lighter is-small`}
          >
        ...
        <td>
          <div className="field has-addons">
            <p className="control">
              <button
                className="button is-square xis-borderless is-small"
                placeholder="Delete"
                onClick={() => handleDeleteOneTicket(ticket.ticket_id)}
              >
            ...
            <p className="control">
              {ticket.ticket_status === "CHECKED IN" ? (
                <button
                  ...
                  onClick={() => handleCheckedOutTicket(ticket.ticket_id)}
                >
                ...
                <button
                  ...
                  onClick={() => handleCheckedInTicket(ticket.ticket_id)}
                >
  ...
  function TableSort({ data }) {
    ...
    const toggleAll = () => {
      setSelection((current) =>
        current.length === data.length ? [] : data.map((item) => item.ticket_id)
      );
    };
    ...
    const rows = sortedData.map((ticket, index) => {
      ...
      return (
        <TableRow
          ...
          ticket={ticket}
          checked={selected}
          onCheck={(e) => toggleRow(ticket.ticket_id)}
        />
      );
    });
        ...
        <thead className="is-sticky">
          <tr className="is-clickable">
            ...
            <Th
              ...
              reversed={reverseSortDirection}
              onSort={() => setSorting("ticket_status")}
            >
              Status
            </Th>
            ...
            <th style={{ width: `8em` }}>
              <button
                ...
                className={`button is-borderless is-rounded has-icons-left is-small`}
                onClick={(e) => handleDeleteManyTickets(selection)}
              >
